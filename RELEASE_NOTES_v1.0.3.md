# RTSP Viewer for Ignition — Free Edition v1.0.3

## What's new
- **Fixed duplicate tiles when rotating.** If the same camera was listed more than once in a view,
  rotation could leave stale tiles behind and show it extra times (e.g. a camera appearing three
  times instead of twice). Tiles are now tracked per slot, so every layout — Grid, Quad, 2-Up, and
  the Hero thumbnail strip — shows exactly the cameras you configured, and rotation swaps pages
  cleanly.
- Includes the v1.0.2 Designer placeholder (no feed loading in the Designer) and the v1.0.1
  live-latency drift fix.
